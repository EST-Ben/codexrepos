...
function sh(cmd, args = [], opts = {}) {
    const out = (0, node_child_process_1.spawnSync)(cmd, args, {
...
function failThrow(msg, details) {
    if (details) {
...
function hasScript(name, pkg) {
    return !!pkg?.scripts?.[name];
}
function runScript(script, pm, extraArgs = []) {
    switch (pm) {
...
function detectPM() {
    ...
    if (exists(yarnLock))
        return { pm: "yarn", lockfile: "yarn.lock" };
    if (exists(npmLock))
...
function parseDotEnvLike(filePath) {
    const out = {};
    if (!exists(filePath))
        return out;
...
checks.push({
    ...
    run: () => {
        ...
        for (const k of Object.keys(example)) {
            if (!(k in envReal) && !(k in process.env))
                missing.push(k);
        }
        if (missing.length) {
...
checks.push({
    ...
    run: () => {
        ...
        if (hasScript("typecheck", pkg)) {
            ...
            if (!which("tsc")) {
                warn("No 'typecheck' script and 'tsc' not found. Skipping.");
                return;
            }
            const res = sh("tsc", ["--noEmit"]);
            if (res.status !== 0)
                failThrow("tsc --noEmit failed.\n" + (res.stdout || res.stderr));
