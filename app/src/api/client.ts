...
export async function exportProfile(payload: {
  ...
  const structured = data.slicer_profile_diff;
  const flatParameters = structured?.parameters
    ? flattenParameters(structured.parameters)
    : {};
  const diff = structured?.diff ?? data.diff ?? {};
  return { diff: Object.keys(diff).length ? diff : flatParameters };
}
